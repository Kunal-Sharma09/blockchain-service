...
export class HederaService {
  ...
  private makeClientForNetwork(): Client {
    switch (this.options.network) {
      case 'mainnet':   return Client.forMainnet();
      case 'testnet':   return Client.forTestnet();
      case 'previewnet':return Client.forPreviewnet();
      default:          throw new Error('Invalid network');
    }
  }
  
  private getPrivateClient(accountId: string, privateKey: string): Client {
    return this.makeClientForNetwork().setOperator(accountId, privateKey);
  }
  ...
  async transferHbar(from: string, to: string, amount: number, privateKey: string): Promise<string> {
    ...
    try {
      const client = this.getPrivateClient(from, privateKey);
    ...
    try {
      ...
      while (url && allTransactions.length < limit) { // <-- CHANGE
        ...
        allTransactions.push(
          ...transactions.slice(0, remaining).map((tx: any) => ({
            transactionId: tx.transaction_id,
            status: tx.result,
            memo: tx.memo_base64 ? Buffer.from(tx.memo_base64, 'base64').toString() : 'No memo',
            type: tx.name,
